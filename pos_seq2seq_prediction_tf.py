	...
	for line in open('chunking/train.txt'):
		...
		if line:
			...
			if word not in word2idx:
				...
				word_idx += 1 
			currentX.apppend(word2idx[word])

			if tag not ins tag2idx:
				tag2idx[tag] = tag_idx
				tag_idx += 1 
...
x = tf.nn.embedding_lookup(tfWe, inputs) # N x T x D
x = tf.unstack(x, sequence_length, 1) # list of length T, with each element is a tensor of shape N x D
outputs, states = static_rnn(rnn_unit, x, dtype = tf.float32)

# outputs of shape: T x N x M
outputs = tf.transpose(outputs, (1, 0, 2)) # shape: N x T x M
...
cost_op = tf.reduce_mean(
	tf.nn.sparse_softmax_cross_entropy_with_logits(logits = logits, labels = labels_flat)
	)
